...
use syn::{DeriveInput, MetaItem, NestedMetaItem, Lit, Field, Attribute};
use quote::{Tokens};
...
impl Table {
    fn from_ast(ast: &syn::DeriveInput) -> Self {
        ...
        for key in &primary_keys {
            if columns.iter().find(|c| c.column_name.as_str() == *key).is_none() {
    ...
    fn to_tokens(self) -> Tokens {
        ...
        primary_keys.append_separated(self.primary_keys.into_iter().map(|k| syn::Ident::new(k)), ",");
